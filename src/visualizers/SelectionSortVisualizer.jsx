...
export default function SelectionSortVisualizer() {
  ...
  const handlePlay = async () => {
    ...
    let arr = [...array];
    setMessage(`Starting Selection Sort on [${arr.map(a => a.value).join(', ')}]`);
    await speak(`Starting Selection Sort on [${arr.map(a => a.value).join(', ')}]`);
    await sleep(getDelay());

    for (let i = 0; i < arr.length; i++) {
      let minIdx = i;
      arr[minIdx] = { ...arr[minIdx], state: 'current_min' };
      setArray([...arr]);
      
      setMessage(`Pass ${i + 1}: Searching for minimum in positions ${i} to ${arr.length - 1}`);
      
      await speak(`Pass ${i + 1}: Searching for minimum in positions ${i} to ${arr.length - 1}`);
      await sleep(getDelay());
      ...
      for (let j = i + 1; j < arr.length; j++) {
        ...
        setArray([...arr]);
        setMessage(`Comparing ${arr[j].value} with current minimum ${arr[minIdx].value}...`);
        await speak(`Comparing ${arr[j].value} with current minimum ${arr[minIdx].value}...`);
        await sleep(getDelay());
        ...
        if (arr[j].value < arr[minIdx].value) {
          if (minIdx !== i) {
            ...
            arr[minIdx] = { ...arr[minIdx], state: 'default' }; // Will be re-colored if needed, but it's the start
          }
          minIdx = j;
          arr[minIdx] = { ...arr[minIdx], state: 'current_min' };
          setArray([...arr]);
          setMessage(`New minimum found: ${arr[minIdx].value} at position ${minIdx}`);
          await speak(`New minimum found: ${arr[minIdx].value} at position ${minIdx}`);
          await sleep(getDelay());
        } else {
      ...
      if (minIdx !== i) {
        ...
        setArray([...arr]);
        setMessage(`Swapping minimum ${arr[minIdx].value} with ${arr[i].value} at position ${i}`);
        await speak(`Swapping minimum ${arr[minIdx].value} with ${arr[i].value} at position ${i}`);
        await sleep(getDelay());
      ...
      setArray([...arr]);
      setMessage(`Pass ${i + 1} done! Position ${i} now has correct value ${arr[i].value}`);
      await speak(`Pass ${i + 1} done! Position ${i} now has correct value ${arr[i].value}`);
      await sleep(getDelay());
    }
  ...
  const getColor = (state) => {
    switch (state) {
      ...
      case 'current_min': return '#3B82F6'; // blue
  ...
  return (
    <>
      ...
      <div className="w-full lg:w-[40%] flex flex-col gap-6 sticky top-[20px]">
        ...
        {showQuiz && (
          <QuizMode
            ...
            quizData={quizData["Selection Sort"]}
      ...
      <AIChat algorithmName="Selection Sort" />
    </>
  );
}
