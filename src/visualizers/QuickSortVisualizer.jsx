...
export default function QuickSortVisualizer() {
  ...
  const quickSort = async (arr, low, high) => {
    ...
    if (low < high) {
      ...
      setMessage(`Recursively sorting left and right partitions...`);
      
      await speak(`Recursively sorting left and right partitions...`);
  ...
  const partition = async (arr, low, high) => {
    ...
    setMessage(`Choosing pivot: ${pivotValue} (last element)`);

    await speak(`Choosing pivot: ${pivotValue} (last element)`);
    await sleep(getDelay());
    ...
    for (let j = low; j < high; j++) {
      ...
      if (arr[j].value < pivotValue) {
        setMessage(`${arr[j].value} < pivot ${pivotValue}: marking as lesser (left side)`);
        await speak(`${arr[j].value} < pivot ${pivotValue}: marking as lesser (left side)`);
        arr[j] = { ...arr[j], state: 'lesser' };
        setArray([...arr]);
        await sleep(getDelay());
        ...
        setMessage(`${arr[j].value} > pivot ${pivotValue}: marking as greater (right side)`);
        await speak(`${arr[j].value} > pivot ${pivotValue}: marking as greater (right side)`);
        arr[j] = { ...arr[j], state: 'greater' };
        setArray([...arr]);
        await sleep(getDelay());
      }
    }
    ...
    setMessage(`Placing pivot ${pivotValue} in its correct position!`);
    
    await speak(`Placing pivot ${pivotValue} in its correct position!`);
    arr[i + 1] = { ...arr[i + 1], state: 'sorted' };
    
    for(let k = low; k <= high; k++) {
      if(k !== i + 1 && arr[k].state !== 'sorted') {
  ...
  const handlePlay = async () => {
    ...
    await speak("Starting Quick Sort...");
    await sleep(getDelay());
  ...
  const getColor = (state) => {
    switch (state) {
      case 'pivot': return '#F59E0B'; // yellow/orange
      case 'lesser': return '#3B82F6'; // blue
      case 'greater': return '#EF4444'; // red
  ...
  return (
    <>
      ...
      <AIChat algorithmName="Quick Sort" />
    </>
  );
}
